...
pub fn format_doctor_report(cli_ok: bool, cli_detail: &str, gpg_hint: &str) -> String {
    ...
    if crate::inproc::lib_loaded() {
        lines.push(format!(
            "libnimvault in-process: yes ({})",
    ...
    lines.push(String::new());
    if let Some(a) = crate::constants::audit_log_path() {
        lines.push(format!("audit log: {a}"));
    }
    if crate::constants::read_only_locked() {
